...
function getPreviewForEditor(editor) {
  if (config.singlePreview) {
    ...
    editor instanceof preview_content_provider_1.MarkdownPreviewEnhancedView
  ) {
    return editor;
...
function startPreview(editor) {
  ...
  if (!preview) {
    if (config.singlePreview) {
      preview = new preview_content_provider_1.MarkdownPreviewEnhancedView(
        "mpe://single_preview",
        config
      );
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (files[i].type.startsWith("image")) {
          ...
          if (imageDropAction === "upload") {
            ...
            preview_content_provider_1.MarkdownPreviewEnhancedView.uploadImageFile(
              editor,
              imageFilePath,
            ...
            event.stopPropagation();
            event.preventDefault();
            preview_content_provider_1.MarkdownPreviewEnhancedView.pasteImageFile(
              editor,
              atom.config.get(
