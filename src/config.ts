...
<<<<<<< HEAD
    ...
    this.phantomPath = atom.config.get(
      "markdown-preview-enhanced-with-litvis.phantomPath",
    );
    this.pandocPath = atom.config.get(
    ...
    this.pandocArguments =
      atom.config
        ...
        .map((x) => x.trim())
        .filter((x) => x.length) || [];
    this.latexEngine = atom.config.get(
    ...
    this.scrollSync = atom.config.get(
      "markdown-preview-enhanced-with-litvis.scrollSync",
    );
    this.liveUpdate = atom.config.get(
